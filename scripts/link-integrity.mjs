...
async function probe(url) {
  // Attempts made for this URL. A URL that needs more than one is either
  // flaky or hostile to the first method tried; the summary below reports
  // the count as a CI warning so a retry that happened to succeed does not
  // silently mask breakage that is one bad day away.
  let attempts = 0;
  const attempt = async method => {
    ...
    try {
      ...
      return { status: res.status, finalUrl: res.url, attempts };
    } finally {
      clearTimeout(timer);
    }
  };
...
// URLs that only resolved after a retry. A retry that succeeds still counts:
// flapping between runs is how a link dies unnoticed, and the retry loop
// above would otherwise absorb it without a trace.
const retried = results
  .filter(r => r.attempts > 1)
  .sort((a, b) => b.attempts - a.attempts || b.pages - a.pages);
if (JSON_OUT) {
  console.log(
    JSON.stringify(
      {
...
`
  ...
  if (retried.length) {
    ...
    for (const r of retried.slice(0, 12))
      console.log(
        `  ${r.attempts} attempts ${r.url}\n       status ${r.status || "ERR"} on ${r.pages} page(s)`
      );
    if (retried.length > 12)
      console.log(`  \u2026 ${retried.length - 12} more`);
    console.log("");
    // GitHub Actions annotations: visible in the run summary rather than
    // buried in the log, so a flaky link gets fixed before it goes red.
    if (process.env.GITHUB_ACTIONS) {
      for (const r of retried)
        console.log(
          `::warning title=link-integrity::${r.url} needed ${r.attempts} attempts ` +
            `(final status ${r.status || "error"}) — linked from ${r.pages} page(s)`
        );
    }
  }
  console.log(
    `[integrity] ${broken.length} unreachable, ${redirected.length} redirected, ${botBlocked.length} bot-blocked, ${retried.length} retried`
  );
}
