...
import { ARCHITECTURE_STAGES, MATURITY_TONES } from "@/data/architecture";
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        <picture>
          ...
          <img
            ...
            width={1920}
            height={1080}
            // Above the fold (LCP): fetch eagerly at high priority.
            loading="eager"
            fetchPriority="high"
            className="absolute inset-0 h-full w-full object-cover"
          />
        </picture>
        {/* The canvas stays mounted while scrolled offscreen: the scene
            freezes its render loop via frameloop="never" instead of tearing
            down (and re-creating) the WebGL context on every scroll. */}
        {webglOk && sceneReady ? (
            ...
            return (
              <li key={s.id} className="flex-1 min-w-0">
                ...
                >
                  ...
                  />
                  {/* Only the active stage's label is shown: in this column
                      width the seven labels collide at desktop sizes.
                      Inactive steps keep just the bar; every button still
                      names its stage via aria-label. */}
                  <span
                    aria-hidden={!active}
                    className={`hidden sm:block truncate text-[10px] font-medium uppercase tracking-wider ${
                      active ? "text-white" : "invisible"
                    }`}
                  >
                    {s.shortLabel}
                  </span>
      ...
      {/* Explainer panel — the semantic equivalent of the 3D story.
          Deliberately NOT aria-live: autoplay re-renders this whole panel
          every 3.2s, which is a screen-reader announcement storm. The
          visually-hidden status line below announces only the meaningful
          change — which step is showing. */}
      <div className="px-5 sm:px-6 mt-4 pb-5 sm:pb-6">
        ...
        {/* Screen-reader status for the autoplay stepper: announces only the
            meaningful change (which step is showing), not the whole panel. */}
        <p aria-live="polite" className="sr-only">
          Step {stage + 1} of {TOTAL_STEPS}: {current.label}
        </p>
      </div>
    </div>
  );
