...
const LAYERS = [
  { id: "hw",   label: "Hardware",     sublabel: "52+ Platforms",    icon: Cpu,     color: "#22D3EE", delay: 0 },
  { id: "boot", label: "eBoot",        sublabel: "Secure Bootloader", icon: Shield,  color: "#F97316", delay: 0.12 },
  { id: "kern", label: "EoS Kernel",   sublabel: "RTOS Core",         icon: Layers,  color: "#A78BFA", delay: 0.24 },
  { id: "plat", label: "Platform",     sublabel: "EIPC · eSec · eNet",icon: Zap,     color: "#34D399", delay: 0.36 },
  { id: "apps", label: "Applications", sublabel: "60+ eApps",         icon: Package, color: "#F59E0B", delay: 0.48 },
  { id: "conn", label: "Connectivity", sublabel: "WiFi · BLE · CAN",  icon: Wifi,    color: "#60A5FA", delay: 0.60 },
];
...
function SignalLine({ color, delay }: { color: string; delay: number }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ duration: 0.5, delay: delay + 0.3, ease: [0.23, 1, 0.32, 1] }}
          ...
          return (
            <div key={layer.id} className="flex items-center flex-1 min-w-0">
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: layer.delay, ease: [0.23, 1, 0.32, 1] }}
              ...
              >
                <div
                  ...
                  style={{
                    background: isActive ? `${layer.color}25` : `${layer.color}10`,
                ...
                <div className="text-center">
                  <div className="text-xs font-bold text-white leading-tight">{layer.label}</div>
                  <div className="text-[9px] text-white/40 leading-tight mt-0.5">{layer.sublabel}</div>
                </div>
              ...
              {i < LAYERS.length - 1 && (
                <div className="flex items-center flex-1 px-1">
                  ...
                  <motion.div
                    ...
                    className="text-xs shrink-0">▶</motion.div>
                </div>
              )}
            </div>
          );
          ...
          return (
            <div key={layer.id}>
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.4, delay: layer.delay, ease: [0.23, 1, 0.32, 1] }}
              ...
              {i < LAYERS.length - 1 && (
                <div className="flex justify-center py-0.5">
                  <div className="w-0.5 h-4" style={{ background: `${LAYERS[i + 1].color}30` }} />
                </div>
              )}
            </div>
          );
