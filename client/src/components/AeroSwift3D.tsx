...
// ── Rotor blade ───────────────────────────────────────────────────────────────
function Rotor({ position, speed = 1 }: { position: [number, number, number]; speed?: number }) {
  const ref = useRef<THREE.Group>(null);
  useFrame(({ clock }) => {
    if (ref.current) ref.current.rotation.y = clock.getElapsedTime() * speed * 8;
  });
  return (
    <group ref={ref} position={position}>
      ...
      {[0, Math.PI / 2, Math.PI, (3 * Math.PI) / 2].map((angle, i) => (
        <mesh key={i} position={[Math.cos(angle) * 0.4, 0, Math.sin(angle) * 0.4]} rotation={[0, angle, 0]}>
          <boxGeometry args={[0.7, 0.025, 0.12]} />
          <meshStandardMaterial color="#1e2540" metalness={0.8} roughness={0.2} transparent opacity={0.85} />
        </mesh>
      ))}
    </group>
  );
  ...
  return (
    <group ref={groupRef}>
      ...
      <mesh position={[0.3, 0.3, 0]}>
        ...
        <meshStandardMaterial color="#22D3EE" transparent opacity={0.35} metalness={0.1} roughness={0} />
      </mesh>
      {/* Wings */}
      <mesh position={[0, 0, 0.9]}>
        <boxGeometry args={[1.8, 0.06, 1.2]} />
        <meshStandardMaterial color="#1e2540" metalness={0.8} roughness={0.15} />
      </mesh>
      <mesh position={[0, 0, -0.9]}>
        <boxGeometry args={[1.8, 0.06, 1.2]} />
        <meshStandardMaterial color="#1e2540" metalness={0.8} roughness={0.15} />
      ...
      {[-0.5, 0, 0.5].map((z, i) => (
        <mesh key={i} position={[0, 0.04, z * 0.55]}>
          ...
          <meshStandardMaterial color="#1a3a5c" emissive="#60A5FA" emissiveIntensity={0.3} />
        </mesh>
      ))}
      {/* Rotors — 4 corners */}
      ...
      {/* Thrust nozzles */}
      <Cone args={[0.08, 0.3, 8]} position={[-1.55, 0, 0.3]} rotation={[0, 0, Math.PI / 2]}>
  ...
  return (
    <group ref={groupRef}>
      ...
      <RoundedBox args={[3.5, 0.9, 1.4]} radius={0.25} smoothness={4}>
        <meshStandardMaterial color="#0d1424" metalness={0.85} roughness={0.12} />
      </RoundedBox>
      {/* Passenger windows */}
      ...
      <mesh position={[1.5, 0.4, 0]}>
        ...
        <meshStandardMaterial color="#22D3EE" transparent opacity={0.3} metalness={0.1} roughness={0} />
      </mesh>
      {/* Wings */}
      <mesh position={[0, 0, 1.4]}>
        <boxGeometry args={[2.5, 0.08, 1.8]} />
        <meshStandardMaterial color="#1a2035" metalness={0.8} roughness={0.15} />
      </mesh>
      <mesh position={[0, 0, -1.4]}>
        <boxGeometry args={[2.5, 0.08, 1.8]} />
        <meshStandardMaterial color="#1a2035" metalness={0.8} roughness={0.15} />
      ...
      {[
        [1.0, 0.55, 1.3], [-1.0, 0.55, 1.3],
        [1.0, 0.55, -1.3], [-1.0, 0.55, -1.3],
        [0, 0.55, 1.3], [0, 0.55, -1.3],
      ].map((pos, i) => (
        <Rotor key={i} position={pos as [number, number, number]} speed={i % 2 === 0 ? 1.1 : -1.0} />
      ))}
      {/* Landing gear */}
      {[-1.2, 1.2].map((x, i) => (
        <Cylinder key={i} args={[0.05, 0.05, 0.5, 8]} position={[x, -0.7, 0]} rotation={[0, 0, 0]}>
          <meshStandardMaterial color="#555" metalness={0.9} roughness={0.2} />
        </Cylinder>
      ))}
      {/* Thrust nozzles */}
      <Cone args={[0.1, 0.4, 8]} position={[-1.9, 0, 0.5]} rotation={[0, 0, Math.PI / 2]}>
...
export function AeroSwiftPersonalCanvas() {
  return (
    <Canvas camera={{ position: [0, 1.5, 6], fov: 45 }} gl={{ antialias: true, alpha: true }} style={{ background: "transparent" }}>
...
export function AeroSwiftTransitCanvas() {
  return (
    <Canvas camera={{ position: [0, 2, 8], fov: 45 }} gl={{ antialias: true, alpha: true }} style={{ background: "transparent" }}>
      <ambientLight intensity={0.3} />
...
export function FlightTelemetry({ model }: { model: "personal" | "transit" }) {
  const data = model === "personal"
    ? { altitude: "1,200 ft", speed: "185 mph", range: "280 mi", battery: "94%", rotors: 4, passengers: 2 }
    : { altitude: "2,800 ft", speed: "210 mph", range: "320 mi", battery: "88%", rotors: 8, passengers: 10 };
