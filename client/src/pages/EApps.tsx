...
import { ArrowRight, Package, Globe, FileText, Database, Code, Gamepad2, Smartphone, Monitor, Puzzle } from "lucide-react";
...
const APP_CATEGORIES = [
  {
    ...
    apps: ["eWriter (Word Processor)", "eSheet (Spreadsheet)", "ePresent (Slides)", "eNotes (Markdown)", "eDraw (Vector Graphics)", "eCalc (Calculator)", "eCalendar", "eContacts", "eMail Client", "eChat", "eFiles (File Manager)"],
  },
  {
    ...
    apps: ["eBrowser (Core)", "Chrome Extension", "Firefox Extension", "VS Code Extension", "JetBrains Plugin", "Obsidian Plugin", "Slack App", "Edge Extension", "Safari Extension", "eBot Web Extension", "DevTools Panel"],
  },
  {
    title: "Desktop Apps",
    count: 4,
    ...
    apps: ["EoStudio (Universal IDE v3.1)", "EoSim (Board Simulator)", "eBrowser Desktop", "eOffice Desktop"],
  },
  {
    title: "Mobile Apps",
    count: 5,
  ...
  },
  {
    ...
    apps: ["System Monitor", "Network Config", "File Browser", "Terminal Emulator", "Media Player", "Image Viewer", "Text Editor", "Calculator", "Clock/Timer", "Weather", "Maps (Offline)", "Camera Viewer", "Sensor Dashboard", "Log Viewer", "OTA Updater", "Settings App", "App Store Client", "Diagnostics", "Benchmark Tool", "Crypto Wallet", "QR Scanner", "Barcode Reader", "PDF Viewer", "eBook Reader", "Music Player", "Video Player", "Photo Gallery", "Contact Book", "Calendar", "Task Manager", "Notes", "Voice Recorder", "Alarm", "Stopwatch", "Unit Converter", "Currency Converter", "Password Manager", "2FA Authenticator", "SSH Client", "FTP Client"],
  },
  {
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <motion.div variants={fadeUp} initial="hidden" animate="visible">
            ...
            <p className="text-white/60 text-lg max-w-2xl mx-auto mb-8">
              Native LVGL apps, browser extensions, desktop apps, and mobile apps —
              all built on EmbeddedOS and available through the unified eApps store.
            </p>
            <a
            ...
            {/* 3D eApps Animation */}
            <div className="mt-10 rounded-2xl border border-white/8 overflow-hidden h-56 max-w-lg mx-auto"
              style={{ background: "rgba(5,10,20,0.8)" }}>
              <Suspense fallback={<div className="w-full h-full flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#F97316] border-t-transparent rounded-full animate-spin" /></div>}>
      ...
      <section className="section-padding bg-[#080F1E]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-10">
            <h2 className="font-heading font-extrabold text-2xl sm:text-3xl text-white mb-2">Featured Applications</h2>
            ...
            {[
              {
                ...
                features: ["AI code completion (eBot)", "RTOS task visualizer", "JTAG/SWD debugger", "One-click board flash", "52+ board profiles", "eFlow visual editor"],
              ...
              },
              {
                ...
                features: ["63+ board profiles", "GPIO/UART/SPI/I2C emulation", "Memory map viewer", "CI/CD integration", "Waveform recorder", "Fault injection testing"],
              ...
              },
              {
                ...
                features: ["eWriter (Word Processor)", "eSheet (Spreadsheet)", "ePresent (Slides)", "eNotes (Markdown)", "eDraw (Vector)", "eMail + eChat"],
              ...
              },
              {
                ...
                features: ["Built-in ad blocker", "eBot AI sidebar", "Offline / air-gapped mode", "11 extensions", "VS Code + JetBrains plugins", "Tor-compatible"],
                platforms: ["Windows", "macOS", "Linux", "EmbeddedOS"],
                href: "https://github.com/embeddedos-org/eApps",
              },
            ].map((app, i) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-0.5">
                      <h3 className="font-heading font-bold text-white text-base">{app.name}</h3>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: app.color + "20", color: app.color }}>{app.version}</span>
                    </div>
                    <p className="text-xs font-semibold" style={{ color: app.color }}>{app.tagline}</p>
                  </div>
                </div>
                <p className="text-sm text-white/55 leading-relaxed">{app.desc}</p>
                <div className="flex flex-wrap gap-1.5">
                  {app.features.map(f => (
                    <span key={f} className="text-[11px] px-2.5 py-1 rounded-lg bg-white/5 text-white/60 border border-white/10">{f}</span>
                  ))}
                </div>
                <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5">
                  <div className="flex gap-1.5">
                    {app.platforms.map(p => (
                      <span key={p} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: app.color + "15", color: app.color }}>{p}</span>
                    ))}
                  </div>
                  <a href={app.href} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold hover:underline" style={{ color: app.color }}>View on GitHub →</a>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-12 bg-[#080F1E]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          >
            ...
            <div>
              <h3 className="font-heading font-bold text-white text-lg mb-2">Automated CI/CD Builds</h3>
              <p className="text-white/60 text-sm leading-relaxed">
                All 60+ apps are built automatically via GitHub Actions on every commit.
                Binaries are published for Linux, Windows, macOS, Android, iOS, and embedded targets.
                The eApps store client auto-updates from the manifest.
              </p>
            </div>
          </motion.div>
        </div>
