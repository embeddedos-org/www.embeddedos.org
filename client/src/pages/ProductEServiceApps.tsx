...
export default function ProductEServiceApps() {
  return (
    <ProductDetailPage
      ...
      heroImage="/manus-storage/product-eserviceapps_447d95c9.jpg"
      ...
      usageExamples={[
        {
          ...
          scenario: "The eHealth365 Mobile app receives real-time biometric data from a HEALTH-BAND Neuro wristband via BLE and displays it on an EoS tablet.",
      ...
      features={[
        { name: "eSocial", desc: "Social networking client with end-to-end encrypted messaging, offline post drafting, and eDB sync." },
        { name: "eRide", desc: "Ride-sharing and navigation app with offline maps, route optimization, and trip logging in eDB." },
        { name: "eTravel", desc: "Travel planning app with offline itineraries, boarding pass storage, and currency conversion." },
        { name: "eWallet", desc: "Digital wallet with NFC payments, transaction history in eDB, and eVault-encrypted credentials." },
        { name: "eHealth365 Mobile", desc: "Companion app for eHealth365 devices. Real-time biometric dashboard, AI health insights, and HIPAA-compliant eDB storage." },
        { name: "Offline-First", desc: "All apps write to eDB first and sync to cloud when connectivity is available." },
        { name: "E2E Encryption", desc: "All user data is encrypted with AES-256 keys stored in eVault." },
        { name: "BLE + WiFi", desc: "Connect to eHealth365 devices via BLE; sync to cloud via WiFi or cellular." },
      ]}
      specs={[
        { key: "Apps", value: "eSocial, eRide, eTravel, eWallet, eHealth365 Mobile" },
        { key: "Architecture", value: "Offline-first with eDB local storage and background cloud sync" },
        { key: "Encryption", value: "AES-256 for data at rest (eVault); TLS 1.3 for data in transit" },
        { key: "Connectivity", value: "BLE 5.0, WiFi 802.11ac, LTE (via modem HAL)" },
        { key: "Health Standards", value: "HIPAA-compliant data handling for eHealth365 Mobile" },
        { key: "License", value: "MIT" },
      ]}
      pairs={[
        { name: "eDB", route: "/product-edb", desc: "Offline-first local storage for all service app data." },
        { name: "eOffice Suite", route: "/product-eoffice", desc: "eVault (part of eOffice) provides secure credential storage for service apps." },
        { name: "eAI", route: "/product-eai", desc: "AI features: health analysis, route optimization, fraud detection." },
      ]}
    />
  );
}
