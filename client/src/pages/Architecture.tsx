...
import {
  Layers, Cpu, Zap, Brain, FileText, Package,
  Database, Shield, Code, Monitor, ChevronRight,
  Heart, ExternalLink, CheckCircle, ArrowRight,
  Radio, GitBranch,
} from "lucide-react";
...
const FULL_STACK_LAYERS = [
  { label: "eServiceApps", sublabels: ["eSocial", "eRide", "eWallet", "eTravel"], color: "#F472B6", y: 2.2, width: 4.0 },
  { label: "eOffice + eApps", sublabels: ["60+ apps", "11 office tools"], color: "#34D399", y: 1.5, width: 3.8 },
  { label: "EAI / ENI", sublabels: ["Neural AI", "BCI", "eDB", "EIPC"], color: "#A78BFA", y: 0.8, width: 3.6 },
  { label: "EoS Kernel", sublabels: ["RTOS", "SMP", "VFS", "IPC"], color: "#F97316", y: 0.1, width: 3.4 },
  { label: "eBoot", sublabels: ["Secure boot", "OTA", "TPM 2.0"], color: "#FBBF24", y: -0.6, width: 3.2 },
  { label: "eCAD Hardware", sublabels: ["15 categories", "KiCad open designs"], color: "#6B7280", y: -1.3, width: 3.0 },
];

// ── 2. EoS Kernel Architecture — LAYERED mode ────────────────────────────────
// Classic horizontal slab stack — the canonical OS diagram
const EOS_LAYERS = [
  { label: "Applications", sublabels: ["eOffice", "eApps", "eServiceApps"], color: "#34D399", y: 1.8, width: 3.8 },
  { label: "Services Layer", sublabels: ["ENI", "EAI", "EIPC", "eDB"], color: "#22D3EE", y: 1.1, width: 3.6 },
  { label: "EoS Kernel", sublabels: ["Scheduler", "MM", "VFS", "IPC"], color: "#F97316", y: 0.4, width: 3.4 },
  { label: "HAL", sublabels: ["GPIO", "SPI", "I2C", "UART", "USB", "CAN"], color: "#A78BFA", y: -0.3, width: 3.2 },
  { label: "Hardware", sublabels: ["ARM Cortex-M/A", "RISC-V", "x86"], color: "#6B7280", y: -1.0, width: 3.0 },
];

// ── 3. eBoot Secure Boot — PIPELINE mode ─────────────────────────────────────
// Left-to-right flow perfectly represents a sequential boot chain
const EBOOT_LAYERS = [
  { label: "ROM Bootrom", sublabels: ["Immutable"], color: "#6B7280", y: 0, width: 1.0 as number },
  { label: "Stage 2: eBoot", sublabels: ["TPM 2.0"], color: "#FBBF24", y: 0, width: 1.0 as number },
  { label: "Stage 3: Verify", sublabels: ["Ed25519"], color: "#F97316", y: 0, width: 1.0 as number },
  { label: "Stage 4: OTA", sublabels: ["A/B slots"], color: "#22D3EE", y: 0, width: 1.0 as number },
  { label: "EoS Kernel", sublabels: ["Handoff"], color: "#34D399", y: 0, width: 1.0 as number },
];

// ── 4. ENI / EAI Neural Pipeline — RADIAL mode ───────────────────────────────
// Hub-and-spoke shows the EAI inference engine at center with I/O nodes orbiting
const ENI_LAYERS = [
  { label: "EAI Inference", sublabels: ["INT4 LLM", "ReAct agents"], color: "#F472B6", y: 0, width: 1.0 as number },
  { label: "Electrode Array", sublabels: ["1,024 channels", "ECoG/Utah"], color: "#EF4444", y: 0, width: 1.0 as number },
  { label: "ADC / Amplifier", sublabels: ["30 kHz", "24-bit"], color: "#F97316", y: 0, width: 1.0 as number },
  { label: "DSP / Spike Sort", sublabels: ["Threshold detect", "PCA"], color: "#22D3EE", y: 0, width: 1.0 as number },
  { label: "Motor Commands", sublabels: ["Stimulation output"], color: "#A78BFA", y: 0, width: 1.0 as number },
  { label: "eHealth365 API", sublabels: ["Biometric stream"], color: "#34D399", y: 0, width: 1.0 as number },
];

// ── 5. eOffice Suite — TREE mode ─────────────────────────────────────────────
// Top-down hierarchy: kernel → services → apps — shows dependency tree
const EOFFICE_LAYERS = [
  { label: "EoS Kernel", sublabels: ["Process isolation", "VFS"], color: "#6B7280", y: 0, width: 1.0 as number },
  { label: "EoS Services", sublabels: ["EIPC", "eDB", "EAI"], color: "#A78BFA", y: 0, width: 1.0 as number },
  { label: "Collab Layer", sublabels: ["CRDT sync", "eBot AI"], color: "#22D3EE", y: 0, width: 1.0 as number },
  { label: "eDocs", sublabels: ["Rich text", "LaTeX"], color: "#34D399", y: 0, width: 1.0 as number },
  { label: "eSheets", sublabels: ["Formulas", "Charts"], color: "#F97316", y: 0, width: 1.0 as number },
  { label: "eSlides", sublabels: ["Decks", "Embed"], color: "#FBBF24", y: 0, width: 1.0 as number },
  { label: "eDrive", sublabels: ["S3", "eVault"], color: "#F472B6", y: 0, width: 1.0 as number },
];

// ── 6. eDB Multi-Model Database — PIPELINE mode ──────────────────────────────
// Left-to-right query path: API → SQL/Doc/KV → Storage
const EDB_LAYERS = [
  { label: "Query API", sublabels: ["REST / tRPC / WS"], color: "#22D3EE", y: 0, width: 1.0 as number },
  { label: "SQL Engine", sublabels: ["B-tree", "WAL", "MVCC"], color: "#34D399", y: 0, width: 1.0 as number },
  { label: "Document Store", sublabels: ["JSON", "BSON", "CBOR"], color: "#F97316", y: 0, width: 1.0 as number },
  { label: "Key-Value Cache", sublabels: ["LRU", "AES-256"], color: "#A78BFA", y: 0, width: 1.0 as number },
  { label: "Storage Engine", sublabels: ["Flash", "NVMe", "SD"], color: "#6B7280", y: 0, width: 1.0 as number },
];

// ── 7. eRadar360 Sensor Fusion — RADIAL mode ─────────────────────────────────
// Hub = Kalman filter fusion, orbiting = individual sensor/processing nodes
const RADAR_LAYERS = [
  { label: "Sensor Fusion EKF", sublabels: ["Kalman filter"], color: "#EF4444", y: 0, width: 1.0 as number },
  { label: "77GHz FMCW Radar", sublabels: ["Range + velocity"], color: "#F97316", y: 0, width: 1.0 as number },
  { label: "LiDAR", sublabels: ["Point cloud 3D"], color: "#FBBF24", y: 0, width: 1.0 as number },
  { label: "Camera", sublabels: ["RGB + depth"], color: "#22D3EE", y: 0, width: 1.0 as number },
  { label: "Perception AI", sublabels: ["YOLO-nano INT4"], color: "#A78BFA", y: 0, width: 1.0 as number },
  { label: "Decision Output", sublabels: ["Object class", "Trajectory"], color: "#34D399", y: 0, width: 1.0 as number },
];
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A] text-white">
      ...
      <section className="relative py-24 px-4 overflow-hidden">
        ...
        />
        <div className={`absolute inset-0 bg-gradient-to-br ${diagram.bgGradient} transition-all duration-700`} />
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed mb-8">
              Seven distinct interactive 3D diagrams — each using a different visual model
              to best represent its product's architecture. From layered OS stacks to radial
              sensor fusion hubs and dependency trees.
            </p>
            <div className="flex flex-wrap justify-center gap-3">
              ...
              >
                <Heart className="w-4 h-4 text-pink-400" /> Support the Foundation
              </Link>
            </div>
          </motion.div>
        </div>
            ...
            {DIAGRAMS.map((d) => {
              ...
              return (
                <button
                  ...
                  style={
                    active === d.id
                      ? { background: d.color + "22", borderColor: d.color + "66", color: d.color }
                      : {}
                  }
                >
                  <Icon className="w-4 h-4" style={{}} />
                  <span className="hidden sm:inline">{d.title.split(" ").slice(0, 2).join(" ")}</span>
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mb-8">
                ...
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <span className="text-xs text-white/30">Visualization:</span>
                    <span
                      className="text-xs font-semibold px-2 py-0.5 rounded-full border"
                      style={{ color: diagram.color, borderColor: diagram.color + "40", background: diagram.color + "12" }}
                    >
                      {MODE_LABELS[diagram.mode]}
                    </span>
                    <span className="text-xs text-white/20">· Drag to rotate · Interactive</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-white mb-3">{diagram.title}</h2>
                  <p className="text-gray-400 leading-relaxed mb-5">{diagram.desc}</p>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                    {diagram.stats.map((s) => (
                      ...
                      >
                        <div className="font-bold text-lg" style={{ color: diagram.color }}>
                          {s.value}
                        </div>
                  ...
                  >
                    Learn more about {diagram.title.split(" ").slice(0, 3).join(" ")}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center mb-8">
                ...
                <div
                  ...
                  style={{ background: diagram.color + "0a", borderColor: diagram.color + "30" }}
                >
                  <div className="flex items-center gap-2 mb-3">
                    <CheckCircle className="w-5 h-5" style={{ color: diagram.color }} />
                    <span className="font-bold text-white text-base">Why This Matters</span>
                  </div>
                  <p className="text-white/70 leading-relaxed text-sm">{diagram.whyMatters}</p>
      ...
      <section className="py-16 px-4 bg-[#080F1E]">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <h2 className="text-3xl font-bold text-white mb-3">All Architecture Diagrams</h2>
            <p className="text-white/50 text-sm max-w-xl mx-auto">
              Each diagram uses a different 3D visualization mode to best represent its product's structure.
              Click any card to open it in the interactive viewer above.
            </p>
          </motion.div>
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: `linear-gradient(90deg, ${d.color}, ${d.color}44)` }}
                  />
      ...
      <section className="py-14 px-4">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            className="text-center mb-10"
          >
            <h2 className="text-2xl font-bold text-white mb-3">5 Visualization Modes</h2>
            <p className="text-white/40 text-sm max-w-xl mx-auto">
              Each diagram type is chosen to match the product's actual architecture pattern.
            </p>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { mode: "layered" as DiagramMode,  color: "#F97316", icon: Layers,    title: "Layered Stack",     desc: "Horizontal slabs showing OS layers from hardware to apps. Best for kernel and platform architectures.", used: "EoS Kernel" },
              { mode: "pipeline" as DiagramMode, color: "#FBBF24", icon: ArrowRight, title: "Pipeline Flow",    desc: "Left-to-right sequential stages with arrows. Best for boot chains and query processing paths.", used: "eBoot, eDB" },
              { mode: "radial" as DiagramMode,   color: "#F472B6", icon: Radio,      title: "Radial Hub",       desc: "Central hub with orbiting nodes. Best for inference engines and sensor fusion systems.", used: "ENI/EAI, eRadar360" },
              { mode: "tree" as DiagramMode,     color: "#34D399", icon: GitBranch,  title: "Dependency Tree",  desc: "Top-down hierarchy showing how components depend on each other. Best for app suites.", used: "eOffice Suite" },
              { mode: "matrix" as DiagramMode,   color: "#A78BFA", icon: Package,    title: "Node Matrix",      desc: "3D grid of glowing nodes. Best for showing the full ecosystem of products at once.", used: "Full Stack" },
            ].map((m, i) => {
              const Icon = m.icon;
              return (
                <motion.div
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-white/30 text-[10px]">Used for: {m.used}</div>
                    </div>
                  </div>
      ...
      <section className="py-16 px-4 bg-[#080F1E]">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            className="text-center mb-10"
          >
            <h2 className="text-3xl font-bold text-white mb-3">Why Donors Fund This Research</h2>
            <p className="text-white/50 text-sm max-w-xl mx-auto">
              Every layer of the EmbeddedOS stack has real-world impact on safety, health, and freedom.
            </p>
          </motion.div>
            ...
            {DONOR_REASONS.map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <h3 className="font-semibold text-white text-base mb-1">{r.title}</h3>
