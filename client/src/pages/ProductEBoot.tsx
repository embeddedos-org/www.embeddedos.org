...
export default function ProductEBoot() {
  return (
    <ProductDetailPage
      ...
      heroImage="/manus-storage/product-eboot_90e7936e.jpg"
      ...
      usageExamples={[
        {
          ...
          scenario: "A fleet of 10,000 industrial sensors receives a firmware update over MQTT with automatic rollback on failure.",
        ...
        },
        {
          ...
          scenario: "A medical device records boot measurements in TPM PCR registers to prove firmware integrity to a remote attestation server.",
      ...
      features={[
        { name: "10 Architectures", desc: "ARMv7-M, ARMv8-A, RISC-V (RV32, RV64), x86_64, AArch64, MIPS, PowerPC, Xtensa, AVR." },
        { name: "24 Reference Boards", desc: "STM32, NXP i.MX, Raspberry Pi, BeagleBone, ESP32, RISC-V SiFive, NVIDIA Jetson, and more." },
        { name: "A/B Update Slots", desc: "Two firmware slots with automatic fallback if a new image fails to mark itself healthy." },
        { name: "Signed Images", desc: "Ed25519 / ECDSA signature verification rooted in immutable boot ROM." },
        { name: "Encrypted Updates", desc: "Optional AES-GCM payload encryption for over-the-air delivery." },
        { name: "Recovery Mode", desc: "Standalone recovery slot with USB / UART / TFTP firmware upload." },
        { name: "Fast Boot", desc: "Sub-second cold boot on Cortex-M, sub-3-second on Cortex-A SoCs." },
        { name: "Watchdog Hand-Off", desc: "Hard watchdog armed before kernel runs; rolls back if kernel doesn't check in." },
      ]}
      specs={[
        { key: "Signature Algorithm", value: "ECDSA-256 (NIST P-256 curve)" },
        { key: "Hash Algorithm", value: "SHA-256" },
        { key: "Encryption", value: "AES-256-GCM for encrypted firmware images" },
        { key: "Anti-Rollback", value: "Monotonic counter in OTP / eFuse; configurable policy" },
        { key: "Update Protocol", value: "OTA-ready; dual-bank A/B update with atomic swap" },
        { key: "Debug Security", value: "JTAG/SWD lock-out in production mode; debug unlock via signed token" },
        { key: "Boot Time (Cortex-M)", value: "< 500 ms cold boot to kernel handoff" },
        { key: "Boot Time (Cortex-A)", value: "< 3 s cold boot to kernel handoff" },
        { key: "Supported Architectures", value: "ARMv7-M, ARMv8-A/R, RISC-V RV32/RV64, x86_64, AArch64, MIPS, PowerPC, Xtensa, AVR" },
        { key: "License", value: "MIT" },
      ]}
      pairs={[
        { name: "EoS", route: "/product-eos", desc: "The foundation: a deterministic real-time embedded OS for everything from the bootloader up." },
        { name: "eBuild", route: "/product-ebuild", desc: "Build system & SDK generator with signing and packaging built-in." },
        { name: "eDB", route: "/product-edb", desc: "Keys rooted in the eBoot chain-of-trust for AES-256 at-rest encryption." },
      ]}
    />
  );
}
