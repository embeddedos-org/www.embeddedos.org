...
import {
  ChevronRight, Radio, Zap, Shield, Wifi, Cpu, MapPin,
  AlertTriangle, Eye, Activity, ArrowRight, ExternalLink,
  Car, Navigation, Lock, Layers
} from "lucide-react";
...
// ─── Sensor Spec Card ────────────────────────────────────────────────────────
function SensorCard({ icon: Icon, label, value, color, delay }: {
  icon: React.FC<{ size?: number; style?: React.CSSProperties }>; label: string; value: string; color: string; delay: number;
}) {
...
function AlertDemo() {
  ...
  const colorMap: Record<string, string> = {
    warn: "#F59E0B", info: "#22D3EE", danger: "#F85149", ok: "#34D399"
  };

  return (
    <div className="rounded-2xl border border-white/10 bg-[#0D1117] p-5">
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-bold text-white/40 uppercase tracking-widest">Live Alert Feed</span>
        <button
        ...
        <AnimatePresence>
          {alerts.map(a => (
            <motion.div
              ...
              style={{ background: `${colorMap[a.type]}15`, borderLeft: `2px solid ${colorMap[a.type]}` }}
            >
...
export default function ERadar360() {
  ...
  const SPECS = [
    { icon: Radio, label: "Front Radar", value: "TI AWR2944 · 77 GHz FMCW · 120° FOV · 0–250 m · 0.75 m resolution", color: "#22D3EE", delay: 0 },
    { icon: Radio, label: "Rear Radar", value: "TI AWR2944 · 77 GHz FMCW · 120° FOV · 0–150 m · 0.75 m resolution", color: "#22D3EE", delay: 0.05 },
    { icon: Zap, label: "Laser Detection", value: "5× Hamamatsu G12183-010K InGaAs APD · 360° (72° spacing) · <50 ms latency", color: "#F97316", delay: 0.1 },
    { icon: Wifi, label: "V2X", value: "Autotalks TEKTON3 DSRC + C-V2X · 1 km LOS · BSM, TIM, SPaT, MAP", color: "#A78BFA", delay: 0.15 },
    { icon: Cpu, label: "AI Processor", value: "Rockchip RK3588S · 6 TOPS NPU · 97% false-alert suppression", color: "#34D399", delay: 0.2 },
    { icon: Cpu, label: "Co-Processor", value: "STM32H7B3 · Laser ADC · OBD-II parsing · GPS fusion", color: "#F59E0B", delay: 0.25 },
    { icon: MapPin, label: "GNSS", value: "u-blox NEO-M9N · GPS + GLONASS + Galileo + BeiDou", color: "#60A5FA", delay: 0.3 },
    { icon: Eye, label: "Display", value: "4\" Samsung AMOLED · 480×800 · MIPI-DSI", color: "#F472B6", delay: 0.35 },
    { icon: Lock, label: "Connectivity", value: "Wi-Fi 6 + BT 5.3 + USB-C · 12V OBD-II or USB-C 5V/3A", color: "#22D3EE", delay: 0.4 },
  ];

  const CERTS = [
    "FCC Part 15B", "FCC Part 15.253 (77 GHz)", "FCC Part 90/95 (V2X)",
    "NHTSA FMVSS 111", "SAE J3016", "ISO 26262 ASIL-B", "IEC 62443", "UNECE WP.29 R155/R156"
  ];

  const CONFIGS = [
    { name: "eRadar360 Standard", desc: "Radar + Laser + GPS", cogs: "$285", msrp: "$699", margin: "59%" },
    { name: "eRadar360 Pro", desc: "Standard + V2X DSRC/C-V2X", cogs: "$310", msrp: "$899", margin: "65%" },
    { name: "Aegis One OEM", desc: "No display, V2X optional", cogs: "$240", msrp: "OEM pricing", margin: "—" },
  ];
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            >
              ...
              <p className="text-xl text-white/60 mb-2 font-semibold">Also marketed as <span className="text-[#A78BFA]">Aegis One</span> (OEM)</p>
              <p className="text-lg text-white/50 mb-8 leading-relaxed">
                360° automotive safety — fusing 77 GHz radar, laser detection, V2X communication, and 6 TOPS AI in a single windshield-mounted device.
              </p>
              <div className="flex flex-wrap gap-3 mb-8">
                {["360° Radar", "Laser Detection", "V2X / DSRC", "6 TOPS AI", "97% Accuracy"].map(tag => (
                  <span key={tag} className="px-3 py-1 rounded-full text-xs font-semibold bg-white/5 text-white/60 border border-white/10">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          >
            <div className="rounded-3xl border border-[#22D3EE]/20 bg-[#0D1117] p-8 w-full max-w-sm">
              <div className="text-center mb-4">
                <span className="text-xs font-bold text-[#22D3EE]/60 uppercase tracking-widest">Live Radar Simulation</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
              {[
                ...
                <div key={s.label} className="rounded-xl border border-white/10 bg-white/5 p-3 text-center">
                  <div className="text-lg font-black text-[#22D3EE]">{s.val}</div>
      ...
      <section className="py-20 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="mb-12"
          >
            <h2 className="text-3xl font-heading font-black mb-3">Sensor Architecture</h2>
            <p className="text-white/50">Four independent sensor modalities fused by a 6 TOPS AI processor.</p>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {SPECS.map(s => <SensorCard key={s.label} {...s} />)}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h2 className="text-3xl font-heading font-black mb-4">Real-Time Threat Detection</h2>
              <p className="text-white/50 mb-6 leading-relaxed">
                The AI processor fuses radar, laser, and V2X data streams in real time. Threats are classified, filtered (97% false-alert suppression), and surfaced to the driver in under 50 ms.
              </p>
              <div className="space-y-3">
                {[
                  { icon: Radio, label: "Radar fusion", desc: "Front + rear 77 GHz FMCW, 0.75 m resolution" },
                  { icon: Zap, label: "Laser grid", desc: "5-point 360° coverage, <50 ms alert latency" },
                  { icon: Wifi, label: "V2X messages", desc: "BSM, TIM, SPaT, MAP at 1 km LOS range" },
                  { icon: Cpu, label: "AI classifier", desc: "RK3588S NPU, 6 TOPS, radar signature fingerprinting" },
                ].map(item => (
      ...
      <section className="py-20 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="mb-12"
          >
            <h2 className="text-3xl font-heading font-black mb-3">Configurations & Pricing</h2>
            <p className="text-white/50">Three configurations for consumer, pro, and OEM markets.</p>
          </motion.div>
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-[#22D3EE] font-bold font-mono">{c.msrp}</span>
                  </div>
                  <div className="flex justify-between">
      ...
      <section className="py-20 border-t border-white/5 bg-[#0D1117]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="mb-10"
          >
            <h2 className="text-3xl font-heading font-black mb-3">Regulatory Compliance</h2>
            <p className="text-white/50">Full documentation complete for all major markets.</p>
          </motion.div>
      ...
      <section className="py-20 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-3xl font-heading font-black mb-4">Explore the CAD Files</h2>
            <p className="text-white/50 mb-8">
              Full KiCad schematics, PCB stackup, BOM, pick-and-place, firmware, and simulation code — all open-source under MIT License.
            </p>
