...
function Lazy3DHero({
  ...
  // Responsive variants: hero-background_1bafea1c-768w.webp etc. The hero
  // renders ~375px wide on mobile, so the 1920px master is 5x oversized.
  // Variants are generated by the image pipeline; fall back to the single
  // source when they don't exist (other posters).
  const stem = poster.replace(/\.(jpe?g|png)$/i, "");
  const ext = poster.match(/\.(jpe?g|png)$/i)?.[0] ?? ".jpg";
  const srcSetWebp = [768, 1280]
    .map(w => `${stem}-${w}w.webp ${w}w`)
    .concat(`${posterWebp} 1920w`)
    .join(", ");
  const srcSetFallback = [768, 1280]
