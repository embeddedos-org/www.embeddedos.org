...
function Lazy3DHero({
  ...
  // Responsive variants (-768w/-1280w) exist only for the stems below —
  // verified against client/public/media. Listing variant srcsets for any
  // other poster would make the browser request files that don't exist
  // (404s), so the variant logic is dropped for posters without them and
  // only the WebP + master fallback remain.
  const KNOWN_VARIANT_STEMS = new Set(["/media/hero-background_1bafea1c"]);
  const stem = poster.replace(/\.(jpe?g|png)$/i, "");
  const ext = poster.match(/\.(jpe?g|png)$/i)?.[0] ?? ".jpg";
  const hasVariants = KNOWN_VARIANT_STEMS.has(stem);
  // The hero renders ~375px wide on mobile, so the 1920px master is 5x
  // oversized; the browser picks the 768w variant on mobile (44KB vs 157KB).
  const srcSetWebp = hasVariants
    ? [768, 1280]
        .map(w => `${stem}-${w}w.webp ${w}w`)
        .concat(`${posterWebp} 1920w`)
        .join(", ")
    : `${posterWebp} 1920w`;
  const srcSetFallback = hasVariants
    ? [768, 1280]
