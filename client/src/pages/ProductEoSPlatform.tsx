...
export default function ProductEoSPlatform() {
  return (
    <ProductDetailPage
      ...
      usageExamples={[
        {
          ...
          scenario: "Rolling out a security patch to 10,000 industrial sensors with automatic rollback on failure.",
      ...
      features={[
        { name: "OTA Firmware Updates", desc: "Signed firmware delivery to device fleets. Staged canary rollouts with automatic rollback." },
        { name: "Fleet Management", desc: "Device registry, group management, and bulk operations via REST API and web dashboard." },
        { name: "Telemetry Pipeline", desc: "Real-time device health metrics (CPU, RAM, temperature, errors) with alerting." },
        { name: "Remote Debug", desc: "Secure GDB tunnel and shell access to any registered device." },
        { name: "Configuration Management", desc: "Push configuration changes to device groups. Version-controlled config history." },
        { name: "REST API", desc: "Full REST API for integration with existing DevOps pipelines (GitHub Actions, Jenkins, etc.)." },
        { name: "A/B Rollout", desc: "Canary deployments with automatic rollback if devices fail to mark firmware healthy." },
        { name: "Audit Log", desc: "Immutable audit log of all platform operations for compliance and forensics." },
      ]}
      specs={[
        { key: "Deployment Model", value: "SaaS (platform.embeddedos.org) or self-hosted (Docker)" },
        { key: "API", value: "REST + WebSocket for real-time telemetry" },
        { key: "Authentication", value: "Device certificates (eBoot-derived); OAuth 2.0 for operators" },
        { key: "Transport Security", value: "TLS 1.3 for all device-to-platform communication" },
        { key: "OTA Protocol", value: "HTTPS with Ed25519-signed firmware images" },
        { key: "Telemetry Retention", value: "90 days (SaaS); configurable (self-hosted)" },
        { key: "License", value: "MIT (self-hosted); SaaS pricing TBD" },
      ]}
      pairs={[
        { name: "eBootloader", route: "/product-eboot", desc: "eBoot's A/B slots are the delivery mechanism for EoS Platform OTA updates." },
        { name: "eBuild", route: "/product-ebuild", desc: "eBuild produces the signed firmware images that EoS Platform distributes." },
        { name: "EoS Kernel", route: "/product-eos", desc: "Device identity and telemetry APIs are provided by the EoS kernel." },
      ]}
    />
  );
}
