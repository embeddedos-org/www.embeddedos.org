...
export default function ProductEAI() {
  return (
    <ProductDetailPage
      badge="AI Runtime"
      title="eAI — On-Device AI Inference Engine"
      subtitle="TFLite · ONNX · GGUF · ReAct Agents · LoRA Fine-tuning"
      description="A full-stack on-device AI runtime for embedded systems. Runs LLMs, vision models, and audio classifiers on NPU, GPU, or CPU — with ReAct agent orchestration, LoRA fine-tuning, and federated learning, all within milliwatt power budgets."
      accent="#A855F7"
      gradient="from-purple-500/20 to-violet-600/20"
      lang="C++ / Python"
      github="embeddedos-org/eAI"
      heroImage="/manus-storage/product-eai-eni_df2d2734.jpg"
      ...
      workflow={[
        {
          step: 1,
          title: "Prepare and Quantize Your Model",
          desc: "Start with a trained model in TensorFlow, PyTorch, or ONNX format. Use eAI's model converter to quantize it to INT8 or 4-bit and generate an .eai bundle with the model, metadata, and calibration data.",
          code: "# Convert and quantize a TFLite model\neai-convert model.tflite --quant int8 --calib calib_data/ --out model.eai\n\n# Or convert from ONNX\neai-convert model.onnx --quant int4 --out model.eai",
        },
        {
          step: 2,
        ...
        {
          step: 4,
          title: "Use ReAct Agents for Multi-Step Reasoning",
          desc: "eAI includes a lightweight ReAct agent loop. Define tools (sensor reads, actuator calls, eDB queries), and the LLM agent will reason and act autonomously — entirely on-device.",
          code: '// Define tools for the agent\neai_tool_t tools[] = {\n    { "read_temp",    tool_read_temperature },\n    { "set_fan_speed", tool_set_fan_speed    },\n    { "query_db",     tool_edb_query        },\n};\n\n// Run the ReAct agent\neai_agent_t agent = eai_agent_create(llm_model, tools, 3);\neai_agent_run(agent,\n    "The server room is overheating. Check the temperature "\n    "and adjust the fan speed to keep it below 25°C.");',
        },
        {
          step: 5,
      ...
      ecosystemRole={{
        ...
        dependsOn: [
          ...
          "EIPC — eAI outputs are routed to actuators and other services via EIPC",
          "eDB — model bundles and training data are stored in eDB",
        ],
        enabledBy: [
          "eHealth365 — arrhythmia detection, SpO₂ analysis, seizure prediction",
          "eNI edge stack — BCI motor decoding, gesture recognition, cognitive load monitoring",
          "eRadar360 — object classification and threat assessment",
          "AeroSwift — flight anomaly detection and predictive maintenance",
          "eOffice — AI-assisted document editing, voice commands, smart scheduling",
        ],
      }}
      features={[
        {
