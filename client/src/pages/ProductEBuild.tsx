...
export default function ProductEBuild() {
  return (
    <ProductDetailPage
      ...
      usageExamples={[
        {
          ...
          scenario: "A GitHub Actions workflow that builds, tests, and signs firmware for every pull request.",
          code: '# .github/workflows/firmware.yml\nname: Firmware CI\non: [push, pull_request]\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - name: Install eBuild\n        run: curl -sSL https://get.embeddedos.org/ebuild | sh\n      - name: Build firmware\n        run: ebuild build --profile stm32f4-discovery\n      - name: Run tests in EoSim\n        run: ebuild test --sim --timeout 60\n      - name: Upload signed firmware\n        uses: actions/upload-artifact@v4\n        with:\n          name: firmware\n          path: build/firmware.eos',
        },
        {
          title: "Multi-Board Workspace",
          scenario: "A monorepo with three boards (sensor, AI, actuator) built and signed in one command.",
      ...
      features={[
        { name: "14 SDK Targets", desc: "Auto-generates a build SDK for every component (EoS, EAI, ENI, EIPC, eBoot, eApps, …)." },
        { name: "Profile Composition", desc: "Combine board + product profile + feature flags; resolves the full dependency graph." },
        { name: "Reproducible Builds", desc: "Pinned source revs, isolated toolchains, hash-stable outputs." },
        { name: "Signing & Packaging", desc: "Built-in Ed25519 signing, .eos / .img / .hex / .uf2 packaging." },
        { name: "Bench & Fuzz", desc: "Integrated micro-benchmark and fuzzer drivers for kernel / app modules." },
        { name: "Workspace Mode", desc: "Multi-repo monorepos with shared toolchain caches." },
        { name: "Static Analysis", desc: "Integrated clang-tidy, cppcheck, and MISRA-C 2012 checker." },
        { name: "Coverage Reports", desc: "GCOV/LCOV coverage reports with HTML output." },
      ]}
      specs={[
        { key: "Build System", value: "CMake 3.25+ with Ninja generator (parallel builds)" },
        { key: "Supported Compilers", value: "GCC 13+, Clang/LLVM 17+, ARM Compiler 6, IAR EWARM, RISC-V GCC, Xtensa GCC" },
        { key: "BSP Profiles", value: "41 pre-configured BSP profiles across 6 categories" },
        { key: "SDK Generation", value: "Automatic SDK generation with headers, libraries, and CMake package config" },
        { key: "Static Analysis", value: "Integrated clang-tidy, cppcheck, and MISRA-C 2012 checker" },
        { key: "Coverage", value: "GCOV/LCOV coverage reports with HTML output" },
        { key: "License", value: "MIT" },
      ]}
      pairs={[
        { name: "EoS Kernel", route: "/product-eos", desc: "eBuild compiles and links the EoS kernel for every supported BSP profile." },
        { name: "eBootloader", route: "/product-eboot", desc: "eBuild signs firmware images with the Ed25519 key that eBoot verifies at boot." },
        { name: "EoStudio", route: "/product-eostudio", desc: "The EoStudio IDE invokes eBuild for all compile, flash, and simulate operations." },
      ]}
    />
  );
}
