...
import { Brain, Wifi, Terminal, Zap, ArrowRight, BookOpen, ChevronRight, Activity, Cpu, Shield } from "lucide-react";
      ...
      <section className="relative py-20 sm:py-28 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 text-center">
          ...
          <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={2}
            ...
            Run AI models directly on microcontrollers. Connect brain-computer interface devices.
            Deploy an AI agent on every EoS device — all without cloud dependency.
          </motion.p>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={4}
            ...
            <Suspense fallback={<div className="w-full h-full flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#34D399] border-t-transparent rounded-full animate-spin" /></div>}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
            ...
            {PRODUCTS.map(p => {
              ...
              return (
                <button key={p.id} onClick={() => setActive(p.id)}
                  ...
                  style={active === p.id
                    ? { background: p.color, color: p.id === "eai" ? "#000" : "#fff" }
          ...
          <motion.div key={active}
            ...
            <div className="rounded-2xl border p-6"
              style={{ background: `${product.color}06`, borderColor: `${product.color}20` }}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="font-heading font-black text-xl text-white">{product.name}</h2>
                  <p className="text-sm" style={{ color: product.color }}>{product.tagline}</p>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-white/8 overflow-hidden"
              ...
              <div className="px-4 py-3 border-b border-white/5 flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-white/30 ml-2">{product.id}-demo.py</span>
              </div>
              <div className="p-5 font-mono text-xs space-y-2">
                {product.demo.map((line, i) => (
                  <motion.div key={i}
                    ...
                    <span style={{
                      color: line.startsWith(">>>") ? product.color
                        : line.startsWith("'") || line.startsWith("{") ? "#34D399"
      ...
      <section className="pb-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
            ...
            <h2 className="font-heading font-black text-3xl text-white mb-2">Use Cases</h2>
            <p className="text-white/40 text-base">What you can build with ENI/EAI</p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { icon: Activity, color: "#F85149", title: "Medical Wearables", desc: "Classify ECG arrhythmias, detect falls, and monitor sleep stages — all on-device with no PHI leaving the hardware." },
              { icon: Brain, color: "#A78BFA", title: "Brain-Computer Interface", desc: "Read motor imagery signals from EEG headsets and translate them into device commands via ENI." },
              { icon: Cpu, color: "#22D3EE", title: "Industrial Predictive Maintenance", desc: "Run vibration anomaly detection models on factory sensors. Alert before failures occur." },
              { icon: Wifi, color: "#34D399", title: "Edge Vision", desc: "Object detection and image classification on camera modules with no cloud upload." },
              { icon: Terminal, color: "#F97316", title: "AI-Assisted Debugging", desc: "eBot reads device logs and suggests fixes in plain language. Ship firmware faster." },
              { icon: Shield, color: "#60A5FA", title: "Secure AI Inference", desc: "Models run in isolated memory partitions. Inference results are signed and attestable." },
            ].map((uc, i) => {
              ...
              return (
                <motion.div key={uc.title} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
                  ...
                  style={{ background: "rgba(255,255,255,0.02)", borderColor: `${uc.color}20` }}>
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3"
                    style={{ background: `${uc.color}15` }}>
                    <UCIcon size={18} style={{ color: uc.color }} />
                  </div>
                  <div className="font-heading font-bold text-white mb-1">{uc.title}</div>
                  <div className="text-white/50 text-sm leading-relaxed">{uc.desc}</div>
                </motion.div>
              );
            })}
          </div>
